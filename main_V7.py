...
class App(tk.Tk):
    ...
    def __init__(self):
        ...
        self.question_types = {
            ...
            "Hard": {
                "Solve the unknown value": {
                    "Question": "{a}{u} {op1_display} {b}{v} {op2_display} {c}{w} {op3_display} {d}{x} {op4_display} {e}{y} {op5_display} {f}{z}",
                    "Display Operators": ["op1", "op2", "op3", "op4", "op5"],
                    "Instruction": "Collect all like terms:",
                    "Random Values": lambda: (
                        lambda letters: {
                            "a": random.randint(1, 9),
                            "b": random.randint(1, 9),
                    ...
                    "Answer": lambda a, b, c, d, e, f, u, v, w, x, y, z, op1, op2, op3, op4, op5: str(sp.simplify(sp.sympify(f"{a}*{u} {op1} {b}*{v} {op2} {c}*{w} {op3} {d}*{x} {op4} {e}*{y} {op5} {f}*{z}"))).replace("*", "")
                }
            }
        }
    ...
    def check_answer(self, user_answer):
        ...
        try:
            ...
            user_expression = parse_expr(user_answer, transformations=implicit_multiplication_application)

            # Convert "**" to "^" and "*" to "" (no text)
            user_answer = str(user_expression).replace("**", "^").replace("*", "")
        except:
            pass
...
class QuizScreen(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.score_label = ttk.Label(right_widgets, font=("TkDefaultFont", 20), style="OutlinedLabels.TLabel")
        ...
        self.question_instruction = ttk.Label(contents_frame, font=("TkDefaultFont", 30))
        self.question_instruction.grid(row=0, column=0, pady=10)

        # Create question label
        self.question_label = ttk.Label(contents_frame, font=("TkDefaultFont", 50))
...
class ResultsScreen(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.title_label = ttk.Label(title_frame, font=("TkDefaultFont", 50))
        self.title_label.pack()

        # Create subheader
        self.subheader_text = ttk.Label(title_frame, font=("TkDefaultFont", 25))
        ...
        # Create main frame header (goes in topbar)
        self.mainframe_header = ttk.Label(topbar, font=("TkDefaultFont", 25))
        ...
        # Create 'Score' header
        score_header = ttk.Label(score_frame, text="Score", font=("TkDefaultFont", 25, "underline"))
        score_header.pack(pady=5)

        # Create 'Score' label
        self.score_label = ttk.Label(score_frame, font=("TkDefaultFont", 25))
        ...
        time_header = ttk.Label(time_frame, text="Time Remaining", font=("TkDefaultFont", 25, "underline"))
        time_header.pack(pady=5)

        # Create 'Time Remaining' label
        self.time_label = ttk.Label(time_frame, font=("TkDefaultFont", 25))
        ...
        datetime_header = ttk.Label(datetime_frame, text="Date & Time", font=("TkDefaultFont", 25, "underline"))
        datetime_header.pack(pady=5)

        # Create 'Date & Time' label
        self.datetime_label = ttk.Label(datetime_frame, font=("TkDefaultFont", 25))
        self.datetime_label.pack(pady=(0, 5))

        # Label for motivational message
        self.motivational_message = ttk.Label(self, font=("TkDefaultFont", 30))

        # Style for 'Return to Lobby' button
        controller.style.configure("ReturnToLobby.TButton", font=("TkDefaultFont", 30), background="#FFFFFF")
