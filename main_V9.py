...
DIFFICULTY_INSTRUCTIONS = {
    ...
    "Hard": (
        ...
        "Make sure to SIMPLIFY your answers if needed to. "
        "It is reccomended to format your answers according to mathmatical conventions as this will help with answering exam-style questions properly (e.g. write x(y + z) instead of (y + z)x)."
    ),
    "Challenge": (
...
def rand_letters(n):
    ...
    return random.sample("abcdfghklmnopqrstuvwxyz", n) # Letters e, i, and j have been removed to prevent SymPy from confusing them to general math symbols.
...
def gen_linear_eq(unknown_position):
    ...
    # Check if unknown's coefficient is 1.
    if unknown_position == "a":
        a_display = "" if a == 1 else a
        b_display = b
    else:
        a_display = a
        b_display = "" if b == 1 else b
...
class App(tk.Tk):
    ...
    def __init__(self):
        ...
        self.question_types = {
            ...
            "Hard": {
                "Solve Unknown #1": {
                    "Question": "{a_display}{x_unknown} {op1_display} {b_display} = {c}",
                ...
                "Solve Unknown #2": {
                    "Question": "{a_display} {op1_display} {b_display}{x_unknown} = {c}",
                    "Display Unknowns": ["x_unknown"],
                    "Display Operators": ["op1"],
                    "Instruction": "Solve for {x_unknown}:",
                ...
                "Factorisation": {
                    "Question": "{a_display}{x_unknown} {op1_display} {b}",
                    "Display Unknowns": ["x_unknown"],
                    "Display Operators": ["op1"],
                ...
                "Expansion": {
                    "Question": "{a}({b_display}{x_unknown} {op1_display} {c})",
                    "Display Unknowns": ["x_unknown"],
                    "Display Operators": ["op1"],
            ...
            "Challenge": {
                "Quadratic Differentiation": {
                    "Question": "{a_display}{x_unknown}² {op1_display} {b_display}{x_unknown} {op2_display} {c}",
                    "Display Unknowns": ["x_unknown"],
                ...
                "Cubic Differentiation": {
                    "Question": "{a_display}{x_unknown}³ {op1_display} {b_display}{x_unknown}² {op2_display} {c_display}{x_unknown} {op3_display} {d}",
                    "Display Unknowns": ["x_unknown"],
                ...
                "Quartic Differentiation": {
                    "Question": "{a_display}{x_unknown}⁴ {op1_display} {b_display}{x_unknown}³ {op2_display} {c_display}{x_unknown}² {op3_display} {d_display}{x_unknown} {op4_display} {e}",
                    "Display Unknowns": ["x_unknown"],
        ...
        for i in range(NUM_QUESTIONS):
            while True:
                ...
                for v in question_data.get("Superscript Values", []):
                    ...
                    if display_values[v] == 1:
                        display_values[f"{v}_display"] = ""
                    else:
    ...
    def check_answer(self, user_answer):
        ...
        if self.selected_difficulty in ["Hard", "Challenge"]:
            try:
                ...
                # Add spaces around the operators for accurate parsing.
                user_answer_formatted = user_answer_formatted.replace("+" , " + ").replace("-", " - ")

                # Convert answer into SymPy expression.
                user_expr = parse_expr(user_answer_formatted, transformations=transformations, evaluate=False)
                ...
                correct_answer_formatted = correct_answer.replace("−", "-")

                # Convert correct answer into SymPy expression format.
                correct_expr = parse_expr(correct_answer_formatted, transformations=transformations, evaluate=False)

                # Get question type.
                question_type = self.questions[self.question_number]["Type"]

                # If question type is factorisation.
                if question_type == "Factorisation":
                    # Check if user's answer is equivalent.
                    if sp.simplify(user_expr - correct_expr) == 0:
                        is_equivalent = True

                    # Check if user's answer is simplified.
                    user_expr_expanded = sp.expand(user_expr)
                    user_expr_factorised = sp.factor(user_expr_expanded)

                    if str(user_expr).replace(" 1*", " ") == str(user_expr_factorised):
                        is_simplified = True

                    # Check if both expressions are equal and if user's answer is simplified
                    if is_equivalent and is_simplified:
                        user_answer = correct_answer

                # If question type is expansion.
                    ...
                    if str(user_expr).replace(" 1*", " ") == str(user_expr_expanded):
                        is_simplified = True
                ...
                # If question type is solving for an unknown or differentiation.
                elif question_type in ["Solve Unknown #1", "Solve Unknown #2", "Quadratic Differentiation", "Cubic Differentiation", "Quartic Differentiation"]:
                    # Check if user's answer is equivalent.
                    if sp.simplify(user_expr - correct_expr) == 0:
                        user_answer = correct_answer
            except:
...
class LoginScreen(tk.Frame):
    def __init__(self, parent, controller):
        ...
        username_label = ttk.Label(login_frame, text="Username:", font=("Arial", 30), background="#FFFFFF", borderwidth=2, relief="solid", padding=(2, 2))
        ...
        password_label = ttk.Label(login_frame, text="Password:", font=("Arial", 30), background="#FFFFFF", borderwidth=2, relief="solid", padding=(2, 2))
...
class SignupScreen(tk.Frame):
    def __init__(self, parent, controller):
        ...
        # Username entry label.
        username_label = ttk.Label(signup_frame, text="Username:", font=("Arial", 30), background="#FFFFFF", borderwidth=2, relief="solid", padding=(2, 2))
        ...
        # Password entry label.
        password_label = ttk.Label(signup_frame, text="Password:", font=("Arial", 30), background="#FFFFFF", borderwidth=2, relief="solid", padding=(2, 2))
        ...
        password_label = ttk.Label(signup_frame, text="Confirm Password:", font=("Arial", 30), background="#FFFFFF", borderwidth=2, relief="solid", padding=(2, 2))
...
class LeaderboardScreen(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.scrollbar.configure(command=self.scores_listbox.yview)

        # Prevent user from being able to interact with records in listbox.
        self.scores_listbox.bind("<<ListboxSelect>>", lambda event: self.scores_listbox.selection_clear(0, tk.END))
...
class PersonalScoresScreen(tk.Frame):
    ...
    def display_scores(self, event=None):
        ...
        user_scores = scores_data.get(self.controller.current_user, [])
...
class InstructionsScreen(tk.Frame):
    def __init__(self, parent, controller):
        ...
        start_btn_frame = tk.Frame(self, background="#FFFFFF", borderwidth=3, relief="solid")
        start_btn_frame.pack(pady=(30, 0), ipadx=5)

        # Create 'Start' button.
        start_btn = ttk.Button(start_btn_frame, text="Start", style="Start.TButton", width=6, command=controller.start_game)
...
class QuizScreen(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.answer_entry.grid(row=0, column=0, padx=5, pady=5)
        ...
        self.submitnext_btn.grid(row=0, column=1, padx=5, pady=5)
...
class ResultsScreen(tk.Frame):
    def __init__(self, parent, controller):
        ...
        # Frame for 'Return to Lobby' button.
        self.return_to_lobby_btn_frame = tk.Frame(self, background="#FFFFFF", borderwidth=3, relief="solid")
        self.return_to_lobby_btn_frame.pack(pady=(30, 0), ipadx=5)

        # Create 'Return to Lobby' button.
        self.return_to_lobby_btn = ttk.Button(self.return_to_lobby_btn_frame, text="Return To Lobby", style="Start.TButton", width=16, command=self.return_to_lobby)
        self.return_to_lobby_btn.pack(pady=5)
    ...
    def display_results(self):
        ...
        self.return_to_lobby_btn_frame.pack_forget()
